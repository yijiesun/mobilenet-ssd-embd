...
int main(int argc, char* argv[])
{
	...
	std::cout << "resize" << std::endl;
        cv::resize(frame, frameImg2, cv::Size(img_h, img_w));
        frameImg2.convertTo(frameImg2, CV_32FC3);
        float* img_data = (float*)frameImg2.data;
	int hw = img_h * img_w;
	...
	while(true)
	{
		...
		cv::resize(frame, frameImg, cv::Size(img_h, img_w));
		frameImg.convertTo(frameImg, CV_32FC3);
		float* img_data = (float*)frameImg.data;
		int hw = img_h * img_w;
		...
		run_graph(graph, 1);
                ...
                tensor_t out_tensor = get_graph_output_tensor(graph, 0, 0);    //"detection_out");tensor_t out_tensor = get_graph_output_tensor(graph, 0, 0);    //"detection_out");
		int out_dim[4];
		...
		for (int i = 0; i < (int)boxes.size(); i++)
		{
			...
			cv::putText(frame, label, cv::Point(box.x0, box.y0), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 0));
                ...
                total_time += mytime;
                std::cout << "--------------------------------------\n";
                std::cout << "avg time per run is " << total_time << " ms\n";
